...
public class UserServiceImpl implements UserService {
    ...
    private UserDetailResponse buildUserDetailResponse(User user) {
        List<ArtistResponse> artists = user.getFollowedArtists().stream().map(catalogEventClient::getArtistById).filter(Objects::nonNull).toList();
        List<EventListResponse> events = user.getFavoriteEvents().stream().map(catalogEventClient::getEventSummaryById).filter(Objects::nonNull).toList();
    ...
    public UserDetailResponse followArtist(String username, FollowArtistRequest request) {
        ...
        if (!user.getFollowedArtists().contains(request.artistId())) {
    ...
    public UserDetailResponse addFavoriteEvent(String username, FavoriteEventRequest request) {
        ...
        if (!user.getFavoriteEvents().contains(request.eventId())) {
    ...
    public List<ArtistResponse> getFollowedArtists(String username) {
        User user = getUserByUsernameOrThrow(username);
        return user.getFollowedArtists().stream().map(catalogEventClient::getArtistById).filter(Objects::nonNull).toList();
    }
    ...
    public List<EventListResponse> getFutureActiveEventsFromFollowedArtistsOrderedByDate(String username) {
        // FIXME: Posible problema de rendimiento si el usuario sigue muchos artistas y cada artista tiene muchos eventos.
        // Se podría optimizar al enviar una lista de IDs de artistas para que devuelva todos los eventos en una sola llamada.
